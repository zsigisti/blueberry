...
- **Package recipes** — the most useful contribution. Add or update a
  `packages/<name>/PKGBUILD`. See [Creating Packages](Creating-Packages).
...
2. Make the change. For a package, build it:
   ```sh
   ENGINE=podman tools/build-pkgs.sh ./out <pkg>
   bpm install ./out/<pkg>-*.pkg.tar.zst    # smoke-test
   ```
...
- [ ] Builds clean with `tools/build-pkgs.sh`.
