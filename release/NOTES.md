...
image, and ships the PAM configuration that console login needs. On an existing
system it is `bpm update && bpm upgrade`; on a fresh install, use this ISO.
The server ISO is boot-tested: root autologin reaches a shell with no failed
units.
...
### Console login PAM config

Adding shadow created `/etc/pam.d/` (for its `passwd`/`chsh`/… helpers) but with
no `login`, `system-auth`, or `other`. Because util-linux `login` is PAM-enabled,
`pam_start` for the login service found the directory but no usable config and
aborted — "PAM failure, aborting: Critical error - immediate abort" — which broke
console and serial autologin at boot. (sshd uses its own authentication, so
remote login was unaffected.) The base now ships a complete PAM stack:
`system-auth` (via `pam_unix`), `login`, `su`, and an `other` fallback that
includes `system-auth`, so no service aborts on a missing config. A new
`make test-login` boots the ISO and asserts the root autologin reaches an actual
shell, so this class of regression is caught before shipping.
